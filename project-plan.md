...
*(These tools will require an LLM API key, which can be stored in `localStorage`)*
...
- **API Key Settings Page (`settings.html`):** A dedicated, simple page to input and save OpenAI/Anthropic/Gemini API keys to `localStorage` so other tools can access them automatically without prompting the user every time.
